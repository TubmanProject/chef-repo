...
healthchecks = node['deploy']['redis-healthcheck'][node.chef_environment]
...
healthchecks.each do |healthcheck|
  ...
  # redis listening port and password
  if defined?(node['redisio']['master']['server']['current']['port'])
    env_vars['REDIS_PORT'] = node['redisio']['master']['server']['current']['port']
    env_vars['REDIS_PASS'] = node['redisio']['master']['server']['current']['requirepass']
  else
    env_vars['REDIS_PORT'] = node['redisio']['slave']['server']['current']['port']
    env_vars['REDIS_PASS'] = node['redisio']['slave']['server']['current']['masterauth']
  end
  ...
  pm2_app['error_file'] = "/var/log/redis/#{name}.stderr.log"
  pm2_app['out_file'] = "/var/log/redis/#{name}.stdout.log"
  pm2_app['watch'] = true
  ...
  nginx_site server_name do
    ...
    variables(
      ...
      port: port,
      ssl_directory: node['redisio']['directories']['ssl'],
      ssl_keyfile_name: node['redisio']['keyfile_name'],
      ssl_certfile_name: node['redisio']['certfile_name']
    )
  end
end
